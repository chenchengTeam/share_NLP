...
const drawChart = (divid, options, datas) => {
  ...
  nodes.unshift({ id: null, node: firstLinks.source });
  ...
  const simulation = d3.forceSimulation()
    ...
    .force('charce', d3.forceManyBody().strength(-30))//节点间的作用力，如果不设置.strength(-60）的话，默认是-30
    .force('collide', d3.forceCollide())//使用默认的半径创建一个碰撞作用力。radius默认所有的节点都为1

  simulation.nodes(nodes)
    .force('link', d3.forceLink(links).distance(250).strength(0.1))
    .restart()
